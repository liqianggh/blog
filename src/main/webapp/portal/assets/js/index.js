		...
		success:function(result){
			...
			var blogList = blogPInfo.list;
		 	...
		 	$.each(hotBlogs ,function(index,value){
			    ...
			    imgUrl ="http://image.blog.com/0e019f0c-7b10-4a29-8f94-99f07d1b9774.jpg";
			    //拼接元素
			    /*<li><img src="image/images/eee.jpg" alt="麻将少年" title="麻将少年" id="wows1_0"/></li>*/
			    ul_ws_images.append("<li><img src='"+imgUrl+"'  alt='"+title+"' title='"+title+"'  id='wows1_"+index+"' /></li>");
...
// $("#div_wow_js").append(" <script type='text/javascript' src='assets/engine/wowslider.js'></script><script type='text/javascript' src='assets/engine/script.js'></script>");
				...
				div_ws_bullets.append("<a href='#' title='"+title+"'><span></span></a>")
		 	})
		 	...
		 	$.each(recBlogs ,function(index,value){
		 		...
		 		var blogId = value.blogId;
     	         ...
     	         var childEle = "<li><p><span class='muted'><a data-action='ding' data-id='1052' class='action'><i class='fa fa-eye'></i><span class='count'>"+viewCount+"</span> 浏览</a></span></p><span class='label label-"+(index+1)+"'>1</span> <a href='/getblogdetail.html?blogid="+blogId+"' title='"+title+"'>"+title+"</a></li>";
		 	...
		 	$.each(blogList,function(index,value){
		 		...
		 		var imgUrl=value.imgHost+value.imgUri;
		 		//todo id没有添加
		 		var blogChild = "<article class='excerpt'><header><a class='label label-important' href='design/material.htm'>"+categoryName+"<i class='label-arrow'></i></a> <h2><a target='_blank' href='3849.html' title='"+title+"'>"+title+" </a></h2></header><div class='focus'> <a target='blank' href='#'><img class='thumb' src='"+imgUrl+"' alt='"+title+"' /></a>	</div> 	<span class='note'>"+summary+"</span>	<p class='auth-span'> <span class='muted'><i class='fa fa-clock-o'></i> "+createTimeStr+"</span> <span class='muted'><i class='fa fa-eye'></i> "+viewCount+"℃</span> <span class='muted'><i class='fa fa-comments-o'></i> <a target='_blank' href='3849.html#comments'>"+commentCount+"评论</a></span><span class='muted'> <a href='javascript:;' data-action='ding' data-id='3849' id='Addlike' class='action'><i class='fa fa-heart-o'></i><span class='count'>"+likeCount+"</span>喜欢</a></span></p> </article>";
		 	...
		 	$.each(tagList,function(index,value){
		 		...
		 		tagscloud.append("<a href='alert("+tagId+")' class='tagc"+getRandom(1,5)+"'>"+tagName+'('+tagCount+')'+"</a>");
		 	...
		 	$.each(hotBlogs,function(index,value){
		 		...
		 		var createTimeStr = value.createTimeStr;
		 		var imgUrl=value.imgHost+value.imgUri;
		 		var childEle =  "<li><a href='"+blogId+"' title='"+title+"'><span class='thumbnail'><img src='"+imgUrl+"' alt='"+title+"' /></span><span class='text'>"+title+"</span><span class='muted'>"+createTimeStr+"</span><span class='muted' style='float: right;'>"+commentCount+"评论</span></a></li>";
