...
function initialNewBlogs(newBlogs){
 	...
 	$.each(newBlogs,function(index,value){
 		...
 		var createTimeStr = value.createTimeStr;
 		var imgUrl=value.imgHost+value.imgUri;
 		var childEle =  "<li><a href='article.html?blogId="+blogId+"' title='"+title+"'><span class='thumbnail'><img src='"+imgUrl+"' alt='"+title+"' /></span><span class='text'>"+title+"</span><span class='muted'>"+createTimeStr+"</span><span class='muted' style='float: right;'><span id = 'url::http://www.mycookies.cn/portal/article.html?blogId="+blogId+"' class = 'cy_cmt_count' ></span><script id='cy_cmt_num' src='https://changyan.sohu.com/upload/plugins/plugins.list.count.js?clientId=cytqlFQwr'></script> 评论</span></a></li>";
